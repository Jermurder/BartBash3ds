...
void texts()
{
C2D_Font font = C2D_FontLoad("romfs:/fonts/Helvetica.bcfnt");

// Create label
	UIText* startLabel = malloc(64);
	UIText_Init(startLabel, "Start", font, startButton.x + 85, startButton.y + 50, 1, C2D_Color32(0,0,0,255));
	startButton.label = startLabel;

	UIText* howtoplay = malloc(64);
	UIText_Init(howtoplay, "How to play", font, howtoplayButton.x + 43, howtoplayButton.y + 50, 1, C2D_Color32(0,0,0,255));
	howtoplayButton.label = howtoplay;
}

void drawTransition() {
    if (!scenemanager.isTransitioning) return;

    scenemanager.transitionProgress += DeltaTime_Get() * 1.0f; // Speed factor
    ...
    if (scenemanager.transitionPhase == TRANSITION_OUT) {
        ...
        C2D_DrawRectSolid(0, 0, 0, SCREEN_WIDTH, SCREEN_HEIGHT, C2D_Color32(0, 0, 0, (u8)(alpha * 255)));
    }
...
// Define the function to handle the start button click
void onStartButtonClick() {
	changeScene(&scenemanager, 1); // Change to game scene
	redrawTop = true;
	redrawBottom = true;
}
...
void loadUI() {
	...
	UIButton_SetHoverSprite(&startButton, 1);   // Optional
	UIButton_SetPressedSprite(&startButton, 0); // Optional

	UIButton_Init(&howtoplayButton, SpriteManager_GetSheet(&spriteManager, "UI2"), 2, (320/2) - 110, 130, 220, 80);
	UIButton_SetHoverSprite(&howtoplayButton, 1);   // Optional
	UIButton_SetPressedSprite(&howtoplayButton, 0); // Optional
...
void musicPlayer() {
	if (scenemanager.currentScene == 0) {
		AudioManager_Play("bort", 1); // Loop background music
	} else if (scenemanager.currentScene == 1) {
		AudioManager_Play("bashs", 0); // Play click sound
	}
}
...
int main(int argc, char* argv[]) {
	...
	texts();
	loadSprites();
	AudioManager_Play("music", -1);  // loop background
	while (aptMainLoop()) {
		DeltaTime_Update();
	...
	if (startButton.label) {
    	UIText_Free(startButton.label);
    	free(startButton.label);
	}
